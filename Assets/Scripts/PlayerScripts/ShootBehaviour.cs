			...
			if (Physics.Raycast(ray, out hit, 500f, shotMask))
			{
				if (hit.collider.transform != this.transform)
				{
					...
					if (hit.collider.gameObject.GetComponent<HealthManager>())
					{
					    ...
					    hit.collider.gameObject.GetComponent<HealthManager>().transform.Find("Blood").gameObject.SetActive(true);
						hit.collider.gameObject.GetComponent<HealthManager>().transform.Find("Blood").GetComponent<ParticleSystem>().Play();
					}
